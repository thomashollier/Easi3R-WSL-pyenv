    ...
    for seq in tqdm(seq_list):
        try:
                ...
                if len(imgs) > 2:
                    if args.use_atten_mask:
                        ...
                        scene = global_aligner(
                            ...
                            use_atten_mask=True, batchify=False,
                        )
                    ...
                    scene = global_aligner(
                        ...
                        sintel_ckpt=args.eval_dataset == 'sintel', use_self_mask=not args.use_gt_mask, sam2_mask_refine=args.sam2_mask_refine,
                        empty_cache=len(imgs) >= 80 and len(pairs) > 600, pxl_thre=args.pxl_thresh, # empty cache to make it run on 48GB GPU
                        use_atten_mask=args.use_atten_mask, batchify=not args.not_batchify,
                    )

                    os.makedirs(f'{save_dir}/{seq}', exist_ok=True)
                    scene.save_dynamic_masks(f'{save_dir}/{seq}')
                    if args.use_atten_mask:
                        scene.save_attention_maps(f'{save_dir}/{seq}')
                    if args.sam2_mask_refine:
                        scene.save_init_fused_dynamic_masks(f'{save_dir}/{seq}')
            ...
            if 'out of memory' in str(e):
                ...
                with open(error_log_path, 'a') as f:
                    f.write(f'Error in sequence {seq}: {str(e)}\n')
                print(f'Error in sequence {seq}, skipping.')
    ...
    if rank == 0:
        with open(f'{save_dir}/_error_log.txt', 'a') as f:
            ...
            for i in range(world_size):
                error_log_file = f'{save_dir}/_error_log_{i}.txt'
                if os.path.exists(error_log_file):  # only read the existing log file
                    with open(error_log_file, 'r') as f_sub:
                        f.write(f_sub.read())
                else:
                    f.write(f'No sequences processed by rank {i}\n')  # optional prompt information
