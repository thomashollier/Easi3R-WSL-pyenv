...
class WarpImage_with_valid_mask(torch.nn.Module):
    def __init__(self):
        super(WarpImage_with_valid_mask, self).__init__()
        self.base_coord = None
    ...
    def warp_image(self, base_coord, img_1, mask_1, flow_2_1):
        B, C, H, W = flow_2_1.shape
        sample_grids = base_coord + flow_2_1.permute([0, 2, 3, 1])
        sample_grids[..., 0] /= (W - 1) / 2
        sample_grids[..., 1] /= (H - 1) / 2
        ...
        warped_image_2_from_1 = F.grid_sample(
            img_1, sample_grids, align_corners=True)
        valid_mask_2_from_1 = F.grid_sample(
            mask_1, sample_grids, align_corners=True)
        return warped_image_2_from_1, valid_mask_2_from_1

    def forward(self, img_1, mask_1, flow_2_1):
        B, _, H, W = flow_2_1.shape
        if self.base_coord is None:
            self.init_grid([H, W], device=flow_2_1.device)
        base_coord = self.base_coord.expand([B, -1, -1, -1])
        return self.warp_image(base_coord, img_1, mask_1, flow_2_1)
