...
class Record3dLoader_Customized:
    ...
    def __init__(self, data_dir: Path, conf_threshold: float = 1.0, foreground_conf_threshold: float = 0.1, no_mask: bool = False, xyzw=True, init_conf=False, mask_morph: int = 0):
        ...
        self.mask_paths = sorted(data_dir.glob("enlarged_dynamic_mask_[0-9]*.png"), key=lambda p: int(p.stem.split("_")[-1]))
        # self.mask_paths = sorted(data_dir.glob("dynamic_mask_[0-9]*.png"), key=lambda p: int(p.stem.split("_")[-1]))
    ...
    def get_frame(self, index: int) -> Record3dFrame:
        ...
        if len(self.mask_paths) == 0:
            mask = np.zeros_like(depth, dtype=onp.bool_)
        else:
